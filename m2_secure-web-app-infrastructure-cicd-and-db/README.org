...
- add bucket name
- hosted zone lookup replace the domain name www by loging
- the policy CloudWatchLogsAccess already exists on the role EC2SSMRole
- ami: aws ssm get-parameter --name "/aws/service/ami-amazon-linux-latest/al2023-ami-kernel-6.1-x86_64" --region us-east-2 --query "Parameter.Value" --output text
...
1. Upload templates to S3:
   #+BEGIN_SRC bash
   aws s3 cp . s3://cf-bucket-123434/$AWS_REGION/ --recursive --exclude "*" --include "*.yml" --include "*.yaml" --region $AWS_REGION
   #+END_SRC
